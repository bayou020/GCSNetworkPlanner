...
SerialPortReader::SerialPortReader()
{
    ...
    if (!QFileInfo::exists(serialPortName)) {
        return;
    }
    ...
    if (!m_serialPort->open(QIODevice::ReadWrite)) {
        qDebug() << QObject::tr("Failed to open port %1, error: %2").arg(serialPortName).arg(m_serialPort->errorString());
    }
    else
    {
...

{
   //qDebug() << "SerialPortReader::handleTimeout";
    if (m_readData.isEmpty()) {
       qDebug() << QObject::tr("No data was currently available for reading from port %1").arg(m_serialPort->portName());
    } else {
...
void SerialPortReader::handleError(QSerialPort::SerialPortError serialPortError)
{
    ...
    if (serialPortError == QSerialPort::ReadError) {
        qDebug() << QObject::tr("An I/O error occurred while reading the data from port %1, error: %2").arg(m_serialPort->portName()).arg(m_serialPort->errorString());
    }
}
