...
MOBILITY="${MOBILITY:-0}"
MOBILITY_RADIUS="${MOBILITY_RADIUS:-80}"
...
EXECUTABLE="$(find_built_executable "$NS3_OUTPUT_DIR" '*uav-secure-lte*' || true)"
if [[ -z "$EXECUTABLE" ]]; then
  echo "[ns3] could not find built LTE scenario executable in $NS3_OUTPUT_DIR" >&2
  exit 1
fi
...
ARGS=(
  ...
  --ulBandwidth="$NS3_LTE_UL_BANDWIDTH"
  --txPower="$NS3_LTE_TX_POWER"
